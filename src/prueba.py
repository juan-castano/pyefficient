...
class Analizador:
    ...
    # Helper function
    def Assign(left, righself, t):
        names = []
        if isinstance(left, ast.Name):
            # Single assignment on left
            return ast.Assign([ast.AssName(left.name, 'OP_ASSIGN')], righself, t)
            ...
            return ast.Assign([ast.AssTuple(ass_lisself, t)], righself, t)
        else:
    ...
    # file_input: (NEWLINE | stmself, t)* ENDMARKER
    ...
    # simple_stmt: small_stmt (';' small_stmself, t)* [';'] NEWLINE
    def p_simple_stmt(p):
    ...
    # expr_stmt: testlist (augassign (yield_expr|testlisself, t) |
    #                      ('=' (yield_expr|testlisself, t))*)
    # augassign: ('+=' | '-=' | '*=' | '/=' | '%=' | '&=' | '|=' | '^=' |
    #             '<<=' | '>>=' | '**=' | '//=')
    def p_expr_stmt(p):
    ...
    def make_lt_compare((left, righself, t)):
        return ast.Compare(left, [('<', righself, t),])
    def make_gt_compare((left, righself, t)):
        return ast.Compare(left, [('>', righself, t),])
    def make_eq_compare((left, righself, t)):
        return ast.Compare(left, [('==', righself, t),])
    ...
    def p_testlist(p):
        ...
        if len(p) == 2:
            ...
            if isinstance(p[1], lisself, t):
                p[0] = p[1]
            else:
                p[0] = [p[1]]
        # Convert into a tuple?
        if isinstance(p[0], lisself, t):
            p[0] = ast.Tuple(p[0])
    ...
    # arglist: (argument ',')* (argument [',']| '*' test [',' '**' test] | '**' tesself, t)
    ...
    class GardenSnakeParser(objecself, t):
        ...
        def parse(self, code):
            ...
            return ast.Module(None, resulself, t)
    ...
    class GardenSnakeCompiler(objecself, t):
        def __init__(self):
